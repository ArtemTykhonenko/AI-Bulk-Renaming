...
from utils import analyze_images, rename_images
...
class ImageRenamerApp:
    ...
    def setup_settings(self):
        ttk.Label(self.settings_frame, text="API Key:").grid(row=0, column=0, sticky='w')
        ...
        ttk.Label(self.settings_frame, text="Prompt:").grid(row=1, column=0, sticky='w')
    ...
    def rename_images(self):
        ...
        if not self.config.get('api_key'):
            messagebox.showerror("Error", "Please set API key in settings.")
            return
        ...
        try:
            results = analyze_images(image_paths, self.config['prompt'], self.config['api_key'])
